...
function validateMenuMap(value){
  ...
  const allowed=["freshFish","crumbed","grilled","addChips","burgers","tacos","packs","snacks","chips"];
  const ids=new Set();
  const sections={};
...
function sectionTerms(section){
  return({freshFish:["beer battered","battered"],crumbed:["crumbed"],grilled:["grilled"],burgers:["burger"],tacos:["taco"],packs:["pack","basket"],snacks:[],chips:["chips","fries"]})[section]||[];
}
...
app.get("/api/admin/persistence/status",auth,async(req,res)=>{
  ...
  res.set("Cache-Control","no-store");res.json({dataDir:DATA_DIR,persistent:DATA_DIR!==__dirname,writable,historyCount,version:VERSION});
});
...
function canonicaliseSections(rawSections){
  const canonical={freshFish:[],crumbed:[],grilled:[],addChips:[],burgers:[],tacos:[],packs:[],snacks:[],chips:[]};
...
app.get("/",(req,res)=>{res.set("Cache-Control","no-store, no-cache, must-revalidate");res.sendFile(path.join(__dirname,"menu-main.html"))});
...
for(const f of ["menu.css","menu.js","menu-main.css","menu-main.js","menu-snacks.css","menu-snacks.js","admin.css","admin.js","bhfc-logo-transparent.png"]){app.get("/"+f,(req,res)=>{res.set("Cache-Control","no-store, no-cache, must-revalidate");res.sendFile(path.join(__dirname,f))})}
